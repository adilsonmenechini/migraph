...
Purpose:
- Create or refresh the local Python runtime used by MiGraph inside `.venv`.

Usage:
- Run directly for runtime setup, or let `scripts/migraph` call it automatically.
...
DEFAULT_PIP_TIMEOUT = os.environ.get("MIGRAPH_PIP_TIMEOUT", "300")
DEFAULT_PIP_RETRIES = os.environ.get("MIGRAPH_PIP_RETRIES", "2")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap the MiGraph runtime environment.")
...
def create_venv(repo_root: Path, quiet: bool) -> Path:
    ...
    if not venv_dir.exists():
        if not quiet:
            print(f"[MiGraph] Creating runtime venv at {venv_dir}")
...
def configured_index_urls() -> list[str]:
    urls: list[str] = []
    configured = os.environ.get("MIGRAPH_PIP_INDEX_URL", "").strip()
    if configured:
...
def install_requirements(repo_root: Path, python_bin: Path, quiet: bool) -> None:
    ...
    if not quiet:
        print(f"[MiGraph] Installing runtime dependencies from {req_path}")
    base_cmd = [
        str(python_bin),
        "-m",
        "pip",
    ...
    for index, (label, install_cmd) in enumerate(commands):
        if not quiet and index > 0:
            print(f"[MiGraph] Default package index failed, retrying via {label.split()[-1]}")
    ...
    raise SystemExit(f"Failed to install MiGraph runtime dependencies: {last_error}")
...
def ensure_runtime(repo_root: Path, quiet: bool) -> Path:
    ...
    if runtime_ready_with_python(repo_root, python_bin):
        if not quiet:
            print(f"[MiGraph] Runtime ready: {python_bin}")
        return python_bin
    install_requirements(repo_root, python_bin, quiet)
    ready, report = runtime_report_with_python(repo_root, python_bin)
    if not ready:
        raise SystemExit(
            "MiGraph runtime bootstrap completed, but required modules are still unavailable: "

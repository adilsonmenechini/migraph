...
Usage:
- Prefer `python scripts/migraph clip ...`.
...
def next_ingest_command(root: Path, normalized_path: Path) -> str:
    ...
    return f"python scripts/migraph ingest --root {root} --source {repo_path}"
...
def download_media(url: str, target: Path) -> bool:
    try:
        ...
        return False
    request = urllib_request.Request(url, headers={"User-Agent": "MiGraph/1.7.2"})
    try:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Clip a webpage, file, or pasted text into the MiGraph inbox.")

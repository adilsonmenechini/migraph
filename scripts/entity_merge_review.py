...
Usage:
- Prefer `python scripts/migraph entity-merge-review ...`.
...
def _load_graph(root: Path) -> dict[str, object]:
    ...
    if not graph_path.exists():
        raise SystemExit("Graph data not found. Run `python scripts/migraph graph --root <wiki-root>` first.")
    try:
...
<head>
  ...
  <title>MiGraph Entity Merge Review</title>
  <style>
    :root {{
      --bg: #0b1020;
      --panel: #121935;
...
<body>
  <main class="shell">
    <h1>MiGraph Entity Merge Review</h1>

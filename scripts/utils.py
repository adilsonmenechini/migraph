...
Usage:
- Imported by other MiGraph scripts; not intended for direct execution.
...
def format_output_serve_lines(
    ...
    *,
    ...
    lines = [
        f"MiGraph output server: {output_http_base(host, port)}",
...
def output_serve_hint_line(root: Path) -> str | None:
    ...
    return (
        "Browse via HTTP: "
        f"run `python scripts/migraph serve --root {display_root_arg(root)}` "
...
def batch_ingest_command(root: Path, quality: str = "ready", limit: int | None = None, dry_run: bool = False) -> str:
    command = f"python scripts/migraph batch-ingest --root {display_root_arg(root)} --quality {quality}"
...
def output_access_lines(root: Path) -> list[str]:
    ...
    if output_home is not None:
        ...
        lines.append(
            "Browse via HTTP: "
            f"`python scripts/migraph serve --root {display_root_arg(root)}` "
            f"-> {output_http_url('index.html')}"
        )
        if viewer_exists and not graph_exists:
            lines.append(f"Next: run `python scripts/migraph graph --root {root}` to generate the graph page.")
        elif graph_exists and not viewer_exists:
            lines.append(f"Next: run `python scripts/migraph viewer --root {root}` to generate the viewer page.")
        return lines

    lines.append(f"Next: run `python scripts/migraph viewer --root {root}` to generate the local viewer page.")
    lines.append(f"Next: run `python scripts/migraph graph --root {root}` to generate the knowledge graph page.")
    return lines

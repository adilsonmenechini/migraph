...
DEFAULT_EMBED_BASE_URL = "http://localhost:11434/v1/embeddings"
...
def resolve_llm_config() -> LlmConfig:
    api_key = _env("MIGRAPH_LLM_API_KEY")
    base_url = _env("MIGRAPH_LLM_BASE_URL")
    model = _env("MIGRAPH_LLM_MODEL")
    if not api_key and not base_url and not model:
        raise RuntimeError(
            "LLM is not configured. Set MIGRAPH_LLM_API_KEY, MIGRAPH_LLM_BASE_URL, "
            "and MIGRAPH_LLM_MODEL to enable OpenAI-compatible content generation."
        )
    missing = [
        name
        for name, value in (
            ("MIGRAPH_LLM_API_KEY", api_key),
            ("MIGRAPH_LLM_BASE_URL", base_url),
            ("MIGRAPH_LLM_MODEL", model),
        )
        if not value
    ]
    if missing:
        raise RuntimeError(
            "Incomplete LLM configuration. Set all of MIGRAPH_LLM_API_KEY, "
            f"MIGRAPH_LLM_BASE_URL, and MIGRAPH_LLM_MODEL. Missing: {', '.join(missing)}"
        )
    _validate_service_url(base_url, "MIGRAPH_LLM_BASE_URL")
    return LlmConfig(api_key=api_key, base_url=base_url, model=model)


def resolve_llm_temperature(kind: str, default_by_kind: dict[str, float]) -> float:
    env_override = _env("MIGRAPH_LLM_TEMPERATURE")
    if env_override:
        try:
            ...
            print(
                f"Warning: MIGRAPH_LLM_TEMPERATURE='{env_override}' is not a valid number; "
...
def embed_is_configured() -> bool:
    return bool(_env("MIGRAPH_EMBED_API_KEY"))
...
def resolve_embed_config() -> EmbedConfig:
    ...
    if base_url_env:
        ...
        base_urls = (DEFAULT_EMBED_BASE_URL,)
    model = _env("MIGRAPH_EMBED_MODEL") or DEFAULT_EMBED_MODEL
    for url in base_urls:
        _validate_service_url(url, "MIGRAPH_EMBED_BASE_URL")

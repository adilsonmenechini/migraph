...
class MiGraphRegressionTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls._previous_allow_private = os.environ.get("MIGRAPH_ALLOW_PRIVATE_URL_FETCH")
        os.environ["MIGRAPH_ALLOW_PRIVATE_URL_FETCH"] = "1"

    @classmethod
    def tearDownClass(cls) -> None:
        if cls._previous_allow_private is None:
            os.environ.pop("MIGRAPH_ALLOW_PRIVATE_URL_FETCH", None)
        else:
            os.environ["MIGRAPH_ALLOW_PRIVATE_URL_FETCH"] = cls._previous_allow_private
    ...
    def test_batch_clip_dry_run_lists_supported_directory_sources(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = run_migraph("batch-clip", "--root", str(root), "--source-dir", str(import_dir), "--dry-run")

            self.assertIn("MiGraph Batch Clip", result.stdout)
    ...
    def test_batch_clip_manifest_clips_mixed_items_and_refreshes_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with serve_directory(site_root) as base_url:
                ...
                result = run_migraph("batch-clip", "--root", str(root), "--manifest", str(manifest))
    ...
    def test_batch_ingest_dry_run_only_lists_ready_items(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("MiGraph Batch Ingest", result.stdout)
    ...
    def test_batch_ingest_respects_limit_and_clears_processed_inbox_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = run_migraph("batch-ingest", "--root", str(root), "--limit", "1")
            ...
            self.assertIn("Output hub: output/index.html", result.stdout)
            self.assertIn("Next: run `python scripts/migraph viewer --root", result.stdout)
    ...
    def test_health_passes_on_fresh_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("MiGraph Health Report", result.stdout)
    ...
    def test_status_command_reports_workspace_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = run_migraph("status", "--root", str(root))

            self.assertIn("MiGraph Status", result.stdout)
    ...
    def test_init_reports_next_output_commands(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("Next: run `python scripts/migraph viewer --root", result.stdout)
            self.assertIn("Next: run `python scripts/migraph graph --root", result.stdout)
    ...
    def test_graph_build_writes_html_viewer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("MiGraph Graph", html_text)
    ...
    def test_graph_report_integrates_with_status_health_and_home(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = run_migraph("graph-report", "--root", str(root))
            entity_review_result = run_migraph("entity-merge-review", "--root", str(root))
            ...
            self.assertIn("MiGraph Graph Report", result.stdout)
            self.assertIn("Graph report: output/graph/report.html", result.stdout)
            self.assertIn("MiGraph Entity Merge Review", entity_review_result.stdout)
            ...
            status_result = run_migraph("status", "--root", str(root))
            ...
            health_result = run_migraph("health", "--root", str(root))
            ...
            self.assertIn("MiGraph Graph Report", report_html)
            ...
            self.assertIn("MiGraph Entity Merge Review", entity_review_html)
    ...
    def test_ingest_writes_structured_knowledge_fields_and_graph_relations(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            write_text(
                ...
                Keep scope visible and reviewable in MiGraph.
                """,
            )
            run_script("init_wiki.py", "--root", str(root), "--title", "Test Wiki")
            write_text(
    ...
    def test_entity_merge_apply_rewrites_entities_and_rebuilds_graph_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            review_before = run_migraph("entity-merge-review", "--root", str(root))
            self.assertIn("Ambiguous Alias Groups: 1", review_before.stdout)

            apply_result = run_migraph(
                "entity-merge-apply",
                "--root",
                str(root),
                "--identity-key",
    ...
    def test_entity_merge_apply_dry_run_writes_plan_without_mutating_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            run_script("build_viewer.py", "--root", str(root))
            run_script("build_graph.py", "--root", str(root))
            run_migraph("entity-merge-review", "--root", str(root))
            ...
            self.assertIn("MiGraph Entity Merge Plan", plan_html)
    ...
    def test_clip_text_creates_inbox_item_and_next_step(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("Inbox review: output/inbox/index.html", result.stdout)
            self.assertIn("Output hub: output/index.html", result.stdout)
            self.assertIn("Next: run `python scripts/migraph ingest --root", result.stdout)
    ...
    def test_clip_refreshes_output_home_and_creates_missing_inbox_dirs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("python scripts/migraph ingest --root", inbox_html)
    ...
    def test_build_inbox_command_creates_review_page(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            inbox_html = (root / "output" / "inbox" / "index.html").read_text(encoding="utf-8")
            self.assertIn("MiGraph Inbox Review", inbox_html)
            self.assertIn("Priority Queue", inbox_html)
            self.assertIn("Ready To Ingest", inbox_html)
            self.assertIn("python scripts/migraph batch-ingest --root", inbox_html)
                ...
                <html>
                  ...
                  <body>
                    <main>
                      ...
                      <p>This article should land in MiGraph inbox with metadata.</p>
                    </main>
                  </body>
                </html>
                """,
    ...
    def test_clip_url_auto_detects_wechat_adapter_from_dom(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            write_text(
                ...
                <html>
                  <body>
                    ...
                    <div id="js_name">MiGraph Channel</div>
                    <div id="js_content">
            ...
            self.assertEqual(payload["siteName"], "MiGraph Channel")
    ...
    def test_clip_url_wechat_code_blocks_are_preserved_for_review(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn('print("hello migraph")', normalized_text)
                    ...
                    <html>
                      ...
                      <body>
                        <main>
                          ...
                          <p>It should be long enough for MiGraph to treat the capture as ready instead of leaving it in a weak or review state.</p>
                        </main>
                      </body>
                    </html>
                    """
    ...
    def test_serve_print_urls_lists_workspace_home(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir) / "wiki"
            write_text(root / ".wiki-schema.md", "# marker")
            write_text(root / "output" / "index.html", "<html><body>MiGraph Outputs</body></html>")
            ...
            try:
                ...
                while time.time() < deadline:
                    try:
                        ...
                        self.assertIn("MiGraph Outputs", body)
                        return
                    except urllib.error.URLError:
                        time.sleep(0.05)
                self.fail("Timed out waiting for MiGraph serve command")
            finally:
                proc.terminate()
                proc.wait(timeout=5)
    ...
    def test_serve_fails_when_output_directory_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = run_migraph("serve", "--root", str(root), "--print-urls", check=False)
    ...
    def test_viewer_prints_serve_hint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("python scripts/migraph serve --root", result.stdout)
            self.assertIn("http://127.0.0.1:8765/index.html", result.stdout)

    def test_new_page_types_pattern_runbook_architecture(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir) / "wiki"
            run_script("init_wiki.py", "--root", str(root), "--title", "New Types Wiki")

            # Create pattern page
            write_text(
                root / "wiki" / "patterns" / "retry-pattern.md",
                """
                ---
                title: Retry Pattern
                type: pattern
                created: 2026-06-21
                updated: 2026-06-21
                summary: A pattern for retry logic.
                tags:
                  - pattern
                confidence: high
                status: active
            ...
            # Create runbook page
            write_text(
                root / "wiki" / "runbooks" / "incident-response.md",
                """
                ---
                ...
                created: 2026-06-21
                updated: 2026-06-21
                summary: Steps for incident response.
                tags:
                  - runbook
                confidence: high
                status: active
            ...
            write_text(
                root / "wiki" / "architectures" / "microservices.md",
                """
                ---
                ...
                created: 2026-06-21
                updated: 2026-06-21
                summary: Microservices architecture overview.
                tags:
                  - architecture
                confidence: high
                status: active
            ...
            # Verify viewer includes new types
            viewer_data = json.loads((root / "output" / "viewer" / "viewer.json").read_text(encoding="utf-8"))
            page_types = {page["type"] for page in viewer_data["pages"]}
            self.assertIn("pattern", page_types)
            self.assertIn("runbook", page_types)
            self.assertIn("architecture", page_types)

            # Verify graph includes new types with correct TYPE_ORDER (all = 2)
            graph_data = json.loads((root / "output" / "graph" / "graph.json").read_text(encoding="utf-8"))
            node_types = {node["type"] for node in graph_data["nodes"]}
            self.assertIn("pattern", node_types)
            self.assertIn("runbook", node_types)
            self.assertIn("architecture", node_types)

    def test_new_types_appear_in_viewer_sections(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir) / "wiki"
            run_script("init_wiki.py", "--root", str(root), "--title", "New Types Wiki")

            write_text(
                root / "wiki" / "patterns" / "test-pattern.md",
                """
                ---
                title: Test Pattern
                type: pattern
            ...
            write_text(
                root / "wiki" / "runbooks" / "test-runbook.md",
                """
                ---
                title: Test Runbook
                type: runbook
            ...
            write_text(
                root / "wiki" / "architectures" / "test-arch.md",
                """
                ---
            ...
            # Check pages carry correct section titles from SECTION_TITLES mapping
            sections_by_type = {page["type"]: page["section"] for page in viewer_data["pages"]}
            self.assertEqual(sections_by_type.get("pattern"), "Patterns")
            self.assertEqual(sections_by_type.get("runbook"), "Runbooks")
            self.assertEqual(sections_by_type.get("architecture"), "Architectures")

    def test_new_types_in_status_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir) / "wiki"
            run_script("init_wiki.py", "--root", str(root), "--title", "Status Wiki")

            write_text(
                root / "wiki" / "patterns" / "p1.md",
                """
                ---
                title: Pattern 1
                type: pattern
            ...
            result = run_migraph("status", "--root", str(root))
            self.assertIn("pattern=1", result.stdout)
            self.assertIn("runbook=1", result.stdout)
            self.assertIn("architecture=1", result.stdout)
            self.assertIn("Pages: 3", result.stdout)


class MiGraphSecurityTest(unittest.TestCase):
    ...
    def test_llm_requires_complete_configuration(self) -> None:
        ...
        try:
            for key in ai_keys:
                del os.environ[key]
            importlib.reload(ai_config)
            os.environ["MIGRAPH_LLM_API_KEY"] = "test-key"
            os.environ["MIGRAPH_LLM_BASE_URL"] = "https://api.example.com/v1/chat/completions"
            os.environ["MIGRAPH_LLM_MODEL"] = "demo-model"
            importlib.reload(ai_config)
    ...
    def test_embed_defaults_to_siliconflow_when_only_key_is_set(self) -> None:
        ...
        try:
            for key in ai_keys:
                del os.environ[key]
            importlib.reload(ai_config)
            os.environ["MIGRAPH_EMBED_API_KEY"] = "test-key"
            importlib.reload(ai_config)
...
class TestDedupeBugFix(unittest.TestCase):
    def test_same_category_same_type_are_compared(self) -> None:
        ...
        title_score = _title_similarity("Kubernetes Patterns", "Kubernetes Patterns")
        self.assertEqual(title_score, 1.0)

        content_score = _content_similarity("Content A", "Content B")
        self.assertLess(content_score, 1.0)
